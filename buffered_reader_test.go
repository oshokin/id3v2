...
func TestReadTillDelim(t *testing.T) {
	...
	bufReader := newBufferedReader(bytes.NewReader(bs))
	...
	read, err := bufReader.readTillDelimiter(55)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReadTillZero(t *testing.T) {
	...
	read, err := bufReader.readTillDelimiter(0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReadTextUTF16WithLeadingEmptyString(t *testing.T) {
	...
	if bufReader.buf.Buffered() != len(sampleTextSuffix) {
		t.Errorf("Expected buffered: %v, got %v", len(sampleTextSuffix), bufReader.buf.Buffered())
	}
...
func TestReadTillDelims(t *testing.T) {
	...
	read, err := bufReader.readTillDelimiters(delims)
	if err != nil {
		t.Fatal(err)
	}
