...
func TestParseOptionsParseFrames(t *testing.T) {
	tag, err := Open(mp3Path, Options{Parse: true, ParseFrames: []string{ArtistFrameDescription, "Title"}})
	if tag == nil || err != nil {
		t.Fatal("Error while opening mp3 file:", err)
	}
...
func TestParseOptionsParseFramesWithSequenceFrames(t *testing.T) {
	...
	tag, err := ParseReader(buf, Options{Parse: true, ParseFrames: []string{ArtistFrameDescription, "Comments"}})
	if err != nil {
	...
	for _, f := range commentFrames {
		...
		case EnglishISO6392Code:
			isEngCommentInFrame = true
		case GermanISO6392Code:
	...
	if !isEngCommentInFrame {
		t.Error("English comment frame is not in tag")
	}
	...
	if tag.Artist() != ArtistFrameDescription {
		t.Errorf("Expected artist: %q, got %q", ArtistFrameDescription, tag.Artist())
	}
...
func TestParseInvalidFrameSize(t *testing.T) {
	...
	// Write tag header.
	err := writeTagHeader(bw, tagHeaderSize+16, 4)
	if err != nil {
	...
	if tag.Title() != TitleFrameDescription {
...
func testMultiTextFrames(t *testing.T, tag *Tag) {
	...
	artistFrames := tag.GetFrames(tag.CommonID(ArtistFrameDescription))
...
func testTextFrames(t *testing.T, tag *Tag) {
	if err := compareTwoStrings(tag.Artist(), ArtistFrameDescription); err != nil {
		t.Error(err)
	}
...
func testUSLTFrames(t *testing.T, tag *Tag) {
	...
	for _, f := range usltFrames {
		...
		if uslf.Language == EnglishISO6392Code {
			parsedEngUSLF = uslf
		}

		if uslf.Language == GermanISO6392Code {
			parsedGerUSLF = uslf
		}
	}
...
func testCommentFrames(t *testing.T, tag *Tag) {
	...
	for _, f := range commFrames {
		...
		if cf.Language == EnglishISO6392Code {
			parsedEngComm = cf
		}

		if cf.Language == GermanISO6392Code {
			parsedGerComm = cf
		}
	}
